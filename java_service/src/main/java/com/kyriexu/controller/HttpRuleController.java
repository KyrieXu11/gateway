...
public class HttpRuleController {
    ...
    public RespBean addHttpService(@RequestBody @Validated HttpRuleInput httpRuleInput) {
        boolean res = httpRuleService.add(httpRuleInput);
        return res ? RespBean.ok("添加成功") : RespBean.error("添加失败");
    }

    @PutMapping("/updateHttpService")
    public RespBean updateHttpService(@RequestBody @Validated HttpRuleInput httpRuleInput) {
